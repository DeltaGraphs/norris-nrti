...
BarChartModel.prototype.updateRecord = function(flowID, index, newRecord) {
	var filteredFlows = this._flows.filter(function(flow) {console.dir(flow.getProperties().ID); console.dir(flowID);console.dir(flow.getProperties().ID === flowID); return flow.getProperties().ID === flowID;});
