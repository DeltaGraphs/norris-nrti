...
function FlowModel(params) {
	...
	if(params.records.isArray()){
		...
		for (var i=0; i<records; i++){
			...
			this._records[i].norrisRecordID=this._ID.concat(date.getTime()).concat(i);
		}
	}
}
...
FlowModel.prototype.updateRecord = function(ID_index, record){
	...
	if (ID_index.indexOf(this._ID) !== -1) {
		...
		if (found){
			...
			this._records[i].norrisRecordID=this._ID.concat(date.getTime()).concat(0);
		}
		else{
		...
		if (typeof ID_index === 'number' && ID_index < this._records.length){
			...
			this._records[ID_index].norrisRecordID=this._ID.concat(date.getTime()).concat(0);
		}
		//invalid ID_index
		return 251;
	}
