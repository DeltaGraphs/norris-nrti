...
MapChartFlowModel.prototype.updateMovie = function(params){
	//aggiungere controllo su tipo records
	var found=false;
	var recordsID=[];
	//this._records=[];
	var i=0;
	for (var keyParam in params) {
		...
		for (var keyRecord in this._records) {
			console.log('['+ params[this._objectKey] +' '+ this._records[keyRecord][this._objectKey] +']');
			if(params[this._objectKey]===this._records[keyRecord][this._objectKey]){
				found=true;
				...
				console.log('\nOLD COPY ID');
				console.dir(this._records[keyRecord]);
				break;
			}
		}
		if(!found){
	...
	/*for (var i=0; i<records.length; i++){
		...
		var found=false;
		var j=0;
		while (j<this._records && !found){
			if(records[this._objectKey]===this._records[j][this._objectKey]){
				found=true;
			}
			else{
				j++;
		...
		var ID;
		if(!found){
			ID=this.generateNorrisRecordID(i);
		}
		else{
			ID=this._records[j].norrisRecordID;
		}
		this._records[i].norrisRecordID=ID;
		recordsID[i]=ID;
		this.validateRecord(i);
	}*/
