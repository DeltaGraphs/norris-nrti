	...
	for (var keyParam in params) {
		if(params[keyParam][this._latitudeKey] === undefined ||
			...
			for (var keyRecord in this._records) {
				found=false;//console.log(params[keyParam][this._objectKey]+' '+this._records[keyRecord][this._objectKey]);
			...
			if(!found){
				...
				//console.log('Nuovo record '+params[keyParam].norrisRecordID);
			}
		}
		i++;
	}
