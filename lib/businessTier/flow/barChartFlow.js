...
BarChartFlow.prototype.updateRecord = function(index, record){
	...
	if (res===true ){
		if (oldValid && !newValid){
            ...
            console.dir(this._dataBarChartFlow.getRecordByIndex(index).norrisRecordID);*/
			var data={
				action: 'deleteRecord',
		...
		if (newValid){
			console.dir('That coverage script is a bit of a douche');
			...
			if (!oldValid){
				console.dir('Here I am, douchebag');
				var data2={
			...
			if (oldValid){
				console.dir('And here I am, you jerk');
				var data3={
					action: 'updateRecord',
					ID: this._dataBarChartFlow._ID,
					norrisRecordID: this._dataBarChartFlow.getRecordByIndex(index).norrisRecordID,
			...
			console.dir('It really is a douche, executed all of this stuff');
		}
	}
	return res;
};
