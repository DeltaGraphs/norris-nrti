...
BarChartFlow.prototype.updateRecord = function(ID, record){
	var oldValid=this._dataBarChartFlow.getRecordByID(ID).norrisRecordIsValid;
	var res=this._dataBarChartFlow.updateRecord(ID, record);
	var newValid=this._dataBarChartFlow.getRecordByID(ID).norrisRecordIsValid;
	if (res===true ){
		...
		if (newValid){
			...
			if (record[indexKey]!==undefined && typeof record[indexKey]==='number'){
				newValue=[record.indexKey];
			}
			if (!oldValid){
				var data2={
...
BarChartFlow.prototype.getReplaceDataJSON = function(){
	...
	for (var i=0; i<dataRecords.length; i++){
		records.push({
			...
			value: [ //lat,long
				dataRecords[i][this._dataBarChartFlow._xKey],
				dataRecords[i][this._dataBarChartFlow._yKey]
			]
		});
	}
